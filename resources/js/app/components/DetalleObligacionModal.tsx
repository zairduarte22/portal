...
export default function DetalleObligacionModal({ isOpen, onClose, obligacion, config, onSuccess }: Props) {
  ...
  const handleDelete = async () => {
    if (!confirm('�Est�s seguro de eliminar esta obligaci�n? Se borrar�n todos los abonos y movimientos bancarios asociados.')) return;
  ...
  const handleDeleteAbono = async (id: number) => {
    if (!confirm('�Seguro que deseas eliminar este abono? Se borrar� el movimiento bancario asociado y se recalcular� la deuda.')) return;
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40">
      <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex justify-between items-center p-6 border-b border-gray-100 sticky top-0 bg-white z-10">
          <div>
            <h2 className="text-xl font-bold text-gray-800">Detalles de Obligaci�n</h2>
            <p className="text-sm text-gray-500">Historial y gesti�n de pagos</p>
          </div>
          ...
          <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100 flex flex-col md:flex-row justify-between gap-6">
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-800">Emisi�n: {new Date(obligacion.fecha_emision + "T12:00:00Z").toLocaleDateString()}</p>
                <p className="text-sm text-gray-800">L�mite: {obligacion.fecha_limite ? new Date(obligacion.fecha_limite + "T12:00:00Z").toLocaleDateString() : 'No aplica'}</p>
              </div>
              {obligacion.descripcion && (
                <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Motivo / Descripci�n</p>
          ...
          <div>
            ...
            {obligacion.abonos && obligacion.abonos.length > 0 ? (
              ...
              <div className="text-center py-8 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                <p className="text-gray-500 text-sm">No hay abonos registrados para esta obligaci�n.</p>
              </div>
            )}
          </div>
        </div>
