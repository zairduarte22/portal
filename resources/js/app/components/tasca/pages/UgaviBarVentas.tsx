          ...
          <div className="flex gap-2">
            <div className="flex flex-col">
              ...
              <input 
                ...
                className="px-3 py-2 border rounded-xl bg-white text-sm outline-none focus:ring-2 focus:ring-green-500"
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border)", color: "var(--muted-foreground)" }}>
                <th className="pb-3 font-semibold text-sm whitespace-nowrap"># Venta</th>
                <th className="pb-3 font-semibold text-sm whitespace-nowrap">Fecha</th>
                <th className="pb-3 font-semibold text-sm whitespace-nowrap min-w-[150px]">Cliente</th>
                <th className="pb-3 font-semibold text-sm whitespace-nowrap">Estado</th>
                <th className="pb-3 font-semibold text-sm whitespace-nowrap">Total (USD)</th>
                <th className="pb-3 font-semibold text-sm text-right whitespace-nowrap">Acciones</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(v => (
                <tr key={v.id} style={{ borderBottom: "1px solid var(--border)" }} className="hover:bg-gray-50/5">
                  <td className="py-3 font-medium whitespace-nowrap">#{v.id}</td>
                  <td className="py-3 text-sm whitespace-nowrap pr-4">{format(new Date(v.fecha), 'dd/MM/yyyy')}</td>
                  <td className="py-3 pr-4 min-w-[180px]">
                    {v.miembro ? (
                      v.persona ? (
                        <div>
                          <span className="font-bold text-gray-900 dark:text-gray-100 whitespace-nowrap">{v.persona.nombre}</span>
                          <br/><span className="text-xs text-gray-500 whitespace-nowrap">{v.miembro.razon_social} (Miembro)</span>
                        </div>
                      ) : (
                        <div>
                          <span className="font-bold text-gray-900 dark:text-gray-100 whitespace-nowrap">{v.miembro.razon_social}</span>
                          <br/><span className="text-xs text-gray-500 whitespace-nowrap">Miembro</span>
                        </div>
                      )
                    ) : v.cliente_foraneo ? (
                      <div>
                        <span className="font-bold text-gray-900 dark:text-gray-100 whitespace-nowrap">{v.cliente_foraneo.nombre}</span>
                        <br/><span className="text-xs text-gray-500 whitespace-nowrap">Foráneo</span>
                      </div>
                    ) : (
                      <span className="text-gray-500 whitespace-nowrap">Desconocido</span>
                    )}
                  </td>
                  ...
                  <td className="py-3 font-bold text-gray-800 dark:text-gray-200 whitespace-nowrap">${(parseFloat(v.total) - parseFloat(v.descuento || "0") + parseFloat(v.cargo_servicio || "0")).toFixed(2)}</td>
