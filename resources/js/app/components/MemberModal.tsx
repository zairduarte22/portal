    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.7)" }}>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-7">
          ...
          {/* Suspensión */}
          <div>
            <SectionTitle>Estado de Suspensión (Congelado)</SectionTitle>
            <div className="rounded-2xl p-4 space-y-4" style={{ backgroundColor: "var(--muted)" }}>
              <Toggle value={!!form.congelado} onChange={(v) => set("congelado", v)} label="Miembro Congelado (No generará cuotas mensuales)" />
              {form.congelado && (
                <div className="pt-2 border-t" style={{ borderColor: "var(--border)" }}>
                  <Field label="Congelado Hasta (Dejar vacío para indefinido)">
                    <input type="date" className={inputBase} style={inputStyle} value={form.congelado_hasta || ""} onChange={(e) => set("congelado_hasta", e.target.value || null)} />
                  </Field>
                </div>
              )}
            </div>
